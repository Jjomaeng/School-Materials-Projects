...
//	Mat input = imread("lena.jpg", CV_LOAD_IMAGE_COLOR);
	...
	if (Subpixel)
	{
		...
		cornerSubPix(input_gray, points,Size_<int>(5,5), Size_<int>(-1, -1), TermCriteria(CV_TERMCRIT_ITER | CV_TERMCRIT_EPS, 10000, 0.0001));
...
Mat NonMaximum_Suppression(const Mat input, Mat corner_mat, int radius)
{
	...
	float max;
		...
		for (int i = radius; i < row ; i++) {
			for (int j = radius; j < col ; j++) {
				if (corner_mat.at<uchar>(i, j) == 1)
				{
					max = input_mirror.at<float>(i, j);
